...
def build():
    ...
    if "label" not in df.columns:
        ...
        score = (0.4*df["demand_score"] + 0.3*df["budget_lakh"] + 0.2*df["seating_capacity"]
                 - 0.1*(df["radius_m"]/10))
        
        # Use percentile-based threshold to ensure balanced classes
        thr = score.quantile(0.4)  # Use 40th percentile instead of median
        
        # Ensure we have both classes by adjusting threshold if needed
        labels = (score > thr).astype(int)
        
        # If we only have one class, adjust the threshold
        if labels.sum() == 0:  # All labels are 0
            thr = score.quantile(0.2)  # Use 20th percentile
            labels = (score > thr).astype(int)
        elif labels.sum() == len(labels):  # All labels are 1
            thr = score.quantile(0.8)  # Use 80th percentile
            labels = (score > thr).astype(int)
        
        # Final check: if still unbalanced, force some labels
        if labels.sum() == 0:
            # Force top 30% to be positive class
            top_indices = score.nlargest(max(1, len(score) // 3)).index
            labels[top_indices] = 1
        elif labels.sum() == len(labels):
            # Force bottom 30% to be negative class
            bottom_indices = score.nsmallest(max(1, len(score) // 3)).index
            labels[bottom_indices] = 0
        
        df["label"] = labels
        
        print(f"[build_dataset] Created weak labels using threshold {thr:.2f}")
        print(f"[build_dataset] Class distribution: {labels.sum()} positive, {len(labels) - labels.sum()} negative")
    else:
